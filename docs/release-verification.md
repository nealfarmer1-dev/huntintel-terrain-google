...
Emulator script: login; draft/quote/local development payment API; processing/library replay; media permission-on-use; offline package download/open; restart; simulated location/navigation; breadcrumb lifecycle; external Google Maps handoff where installed; team share; opt-in foreground SAR; disconnect, queue allowed durable operations, reconnect twice and verify no duplicates; PDF open/share; revoke access and verify future API/download/sync/SAR denial. Confirm background-location permission is absent, foreground-service permissions/configuration remain present, and the persistent visible stop affordance works. Emulator results do not establish physical-device OEM background execution behavior.
