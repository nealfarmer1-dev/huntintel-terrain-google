...
export function AccountScreen({ user, onAuthenticated, onSignedOut, onClose, onReplayOrientation, onOpenDownloads, onOpenAnalyses, appVersion, initialMessage = "" }: Props) {
  ...
  if (mode === "security") {
    ...
    return <SafeAreaView style={styles.safe}><ScrollView keyboardShouldPersistTaps="handled" contentInsetAdjustmentBehavior="automatic" contentContainerStyle={styles.page}>
      <View style={styles.header}><View style={styles.headerCopy}><Text style={styles.eyebrow}>HuntIntel Terrain Intelligence</Text><Text style={styles.title}>Account</Text></View>{onClose && <Button label="Back" accessibilityLabel="Close Account and return to Terrain" onPress={onClose} />}</View>
  ...
  return <SafeAreaView style={styles.safe}><KeyboardAvoidingView style={styles.keyboard} behavior={Platform.OS === "ios" ? "padding" : "height"}><ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={styles.wrap}><View style={styles.card}>
    {mode === "login" ? <View style={styles.loginBrand}><Image accessible accessibilityLabel="HuntIntel Terrain logo" resizeMode="contain" source={TERRAIN_ICON} style={[styles.loginLogo, { width: loginLogoSize, height: loginLogoSize, borderRadius: Math.round(loginLogoSize * (Platform.OS === "ios" ? 0.22 : 0.18)) }]} /><Text style={[styles.eyebrow, styles.loginEyebrow]}>HuntIntel Terrain</Text></View> : <Text style={styles.eyebrow}>HuntIntel Terrain</Text>}
    <Text style={styles.title}>{mode === "login" ? "Sign in" : mode === "register" ? "Create account" : mode === "verify" ? "Verify email" : mode === "forgot" ? "Forgot password" : "Reset password"}</Text>
