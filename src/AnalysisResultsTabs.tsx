...
export function AnalysisResultsTabs({ analysis, analysisJobId, resultsUi, setResultsUi, onSelect, onNavigate, navigationTargetEntity, navigationRequestNonce = 0, navigationRevealNonce = 0, onNavigationRequestVisible, currentLocation, locationFollow, onRequestLocation, onSetFollow, onBreadcrumbChange }: any) {
  const selected = selectedEntity(resultsUi, analysis);
  ...
  return <View style={s.container}>
    ...
    <View style={resultsUi.activeResultsTab === "navigation" ? undefined : s.hidden}><NavigationPanel key={analysisJobId} analysisJobId={analysisJobId} waypoints={waypoints} selectedTarget={navigationTargetEntity} locationRequestNonce={navigationRequestNonce} revealRequestNonce={navigationRevealNonce} onRequestVisible={onNavigationRequestVisible} currentLocation={currentLocation} follow={locationFollow} onRequestLocation={onRequestLocation} onSetFollow={onSetFollow} onBreadcrumbChange={onBreadcrumbChange} onSelectTarget={(waypoint:any)=>{onSelect("waypoint",waypoint.id,false);onNavigate(waypoint)}} /></View>
