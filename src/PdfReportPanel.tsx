...
export function PdfReportPanel({analysisJobId}:{analysisJobId:string}) {
 ...
 return <View style={s.panel}><View style={[s.content,wide&&s.contentWide]}>
  <View style={s.overview}><Text style={s.eyebrow}>VERSIONED PRIVATE ARTIFACT</Text><Text style={s.title}>PDF Report</Text><Text style={s.meta}>Create a polished report from this saved analysis. Optional field records remain separate from HTIE Engine Findings.</Text><View style={s.actions}><Pressable accessibilityRole="button" accessibilityState={{disabled:busy,busy}} style={({pressed})=>[s.primary,busy&&s.disabled,pressed&&s.pressed]} disabled={busy} onPress={generate}><Text style={s.primaryText}>{busy?"Working…":"Generate PDF"}</Text></Pressable>{artifact?.status==="ready"&&<Pressable accessibilityRole="button" accessibilityState={{disabled:busy,busy}} style={({pressed})=>[s.button,busy&&s.disabled,pressed&&s.pressed]} disabled={busy} onPress={share}><Text style={s.buttonText}>Download / Share</Text></Pressable>}</View>{message?<Text accessibilityLiveRegion="polite" style={s.status}>{message}</Text>:null}{artifact?<Text style={s.status}>Status: {artifact.status}{artifact.sizeBytes?` · ${Math.ceil(artifact.sizeBytes/1024)} KiB`:""}</Text>:null}</View>
...
const s=StyleSheet.create({
 ...
 actions:{flexDirection:"row",flexWrap:"wrap",gap:10,marginTop:18},button:{minHeight:48,justifyContent:"center",backgroundColor:"#243025",borderRadius:14,paddingHorizontal:16,paddingVertical:12,borderWidth:1,borderColor:"#3b4b3a"},primary:{minHeight:48,justifyContent:"center",backgroundColor:"#d0a65d",borderRadius:14,paddingHorizontal:16,paddingVertical:12,borderWidth:1,borderColor:"#e5c682"},disabled:{opacity:.5},pressed:{opacity:.76,transform:[{scale:.985}]},buttonText:{color:"#f0f3ea",fontWeight:"700"},primaryText:{color:"#1f180f",fontWeight:"800"},status:{color:"#9cab97",lineHeight:19,marginTop:10},
 includedTitle:{color:"#e6c27a",fontSize:16,fontWeight:"800",marginBottom:14},includedList:{gap:10},includedRow:{flexDirection:"row",alignItems:"flex-start",gap:10,minWidth:0},checkBadge:{width:22,height:22,borderRadius:999,borderWidth:1,borderColor:"#6f9368",backgroundColor:"#1c2a1d",alignItems:"center",justifyContent:"center",marginTop:1},checkBadgeOff:{borderColor:"#354235",backgroundColor:"transparent"},checkText:{color:"#89b37f",fontSize:13,fontWeight:"900",lineHeight:16},checkTextOff:{color:"transparent"},includedText:{flex:1,color:"#f0f3ea",fontSize:15,lineHeight:21},includedTextOff:{color:"#849080"},optionGroup:{gap:10,minHeight:48,justifyContent:"center",padding:8,margin:-8,borderRadius:12}
});
