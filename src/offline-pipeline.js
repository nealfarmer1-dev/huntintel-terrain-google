...
function validatedOfflineMapModel(manifest, layerPreferences) {
  ...
  return { model, map, plan, provider, tiles, immutable, features, relationships, preferences };
}
...
export function renderOfflineMapHtml(manifest, layerPreferences = {}) {
  ...
  const { model, map, plan, provider, immutable, features, relationships, preferences } = validated;
  ...
  return `<!doctype html><html><head><meta name="viewport" content="width=device-width"><style>html,body{margin:0;background:#101510;color:#fff;font:12px sans-serif}#map{position:relative;width:${width}px;height:${height}px;transform-origin:0 0}#analysis-boundary,#analysis-relationships,#field-breadcrumbs{position:absolute;inset:0;pointer-events:none;overflow:visible}#analysis-boundary{z-index:2}#analysis-boundary path{fill:rgba(208,166,93,.18);stroke:#f0d293;stroke-width:4;vector-effect:non-scaling-stroke}#analysis-relationships{z-index:3}#analysis-relationships path{fill:none;stroke:rgba(173,209,255,.82);stroke-width:3;stroke-dasharray:8 6;vector-effect:non-scaling-stroke}#field-breadcrumbs{z-index:4;width:${width}px;height:${height}px}.current-location{display:none;position:absolute;width:16px;height:16px;margin:-8px 0 0 -8px;border:3px solid #fff;border-radius:50%;background:#4f9cff;box-shadow:0 2px 8px #000;z-index:5}.attribution{position:fixed;right:4px;bottom:4px;background:#000b;padding:4px;z-index:6}</style></head><body><div id="map">${images}${boundaryOverlay}${relationshipOverlay}<svg id="field-breadcrumbs" viewBox="0 0 ${width} ${height}" preserveAspectRatio="none"></svg><div id="current-location" class="current-location"></div></div><div class="attribution">${provider.attribution || "Offline map"}</div><script>(function(){var zoom=${zoom},originX=${originX},originY=${originY},marker=document.getElementById('current-location'),crumbs=document.getElementById('field-breadcrumbs');crumbs.style.display=${layerPreferences.field?.breadcrumbs===false?"'none'":"'block'"};function point(location){var size=Math.pow(2,zoom)*256,longitude=Number(location.longitude),latitude=Math.max(-85.0511,Math.min(85.0511,Number(location.latitude))),sin=Math.sin(latitude*Math.PI/180);return{x:(longitude+180)/360*size-originX,y:(.5-Math.log((1+sin)/(1-sin))/(4*Math.PI))*size-originY}}window.__terrainUpdateLocation=function(location){if(!location||!Number.isFinite(Number(location.longitude))||!Number.isFinite(Number(location.latitude)))return false;var p=point(location);marker.style.display='block';marker.style.left=p.x+'px';marker.style.top=p.y+'px';return p};window.__terrainCenterLocation=function(location){var p=window.__terrainUpdateLocation(location);if(!p)return false;window.scrollTo({left:Math.max(0,p.x-window.innerWidth/2),top:Math.max(0,p.y-window.innerHeight/2),behavior:'smooth'});return true};window.__terrainClearLocation=function(){marker.style.display='none'};window.__terrainSetBreadcrumbs=function(data){while(crumbs.firstChild)crumbs.removeChild(crumbs.firstChild);(data&&data.features||[]).forEach(function(feature){var lines=feature.geometry&&feature.geometry.type==='MultiLineString'?feature.geometry.coordinates:[feature.geometry&&feature.geometry.coordinates||[]];lines.forEach(function(line){if(line.length<2)return;var d=line.map(function(coordinate,index){var p=point({longitude:coordinate[0],latitude:coordinate[1]});return(index?'L ':'M ')+p.x+' '+p.y}).join(' '),path=document.createElementNS('http://www.w3.org/2000/svg','path');path.setAttribute('d',d);path.setAttribute('fill','none');path.setAttribute('stroke','#ffbd45');path.setAttribute('stroke-width','4');path.setAttribute('vector-effect','non-scaling-stroke');crumbs.appendChild(path)})})};window.__terrainSelect=function(){return true};})();</script></body></html>`;
}
