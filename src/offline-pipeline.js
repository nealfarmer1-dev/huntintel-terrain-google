...
export function renderOfflineMapHtml(manifest, layerPreferences = {}) {
  ...
  const relationshipFeatures = relationshipsToGeoJson(manifest.immutable?.relationships || [], manifest.immutable?.features || []).features;
  const relationshipPaths = layerPreferences.analysis?.relationships === false ? "" : relationshipFeatures.flatMap((feature) => {
