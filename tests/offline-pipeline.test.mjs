...
const manifest = { analysisJobId: "a", estimatedSizeBytes: 20, immutable: { analysis: { analysisJobId: "a", requestPolygon: boundary }, features: [{ id: "feature-1", geometry: { type: "Point", coordinates: [-86, 36] } }, { id: "feature-2", geometry: { type: "Point", coordinates: [-85.99, 36.01] } }], relationships: [{ id: "relationship-1", sourceFeatureId: "feature-1", targetFeatureId: "feature-2" }], waypoints: [{ id: "waypoint-1", geometry: { type: "Point", coordinates: [-85.995, 36.005] } }] }, map: { region: boundary, tilePlan: { provider: { id: "fixture", attribution: "test fixture" }, maxZoom: 1, tiles: [{ key: "fixture/1/0/0", z: 1, x: 0, y: 0, url: "/tile" }] } }, userRecords: { attachments: [{ id: "att", downloadUrl: "/attachment", contentType: "image/png" }] } };
test("downloads tiles and attachment binaries, checkpoints, retries, and renders offline lookup", async () => { let attempts = 0; const checkpoints = []; const result = await downloadPackageAssets(structuredClone(manifest), { retries: 1, checkpoint: async (value) => checkpoints.push(value.downloadState.status), fetchAsset: async (asset) => { attempts += 1; if (asset.kind === "tile" && attempts === 1) throw new Error("retry"); return { dataUrl: "data:image/png;base64,AA==", sizeBytes: 1, contentType: "image/png" }; } }); assert.equal(attempts, 3); assert.equal(result.downloadState.status, "ready"); assert.ok(offlineTile(result, "fixture", 1, 0, 0)); const html=renderOfflineMapHtml(result);assert.match(html,/data:image\/png/);assert.match(html,/__terrainCenterLocation/);assert.match(html,/post\('map-ready'/);assert.match(html,/__terrainSetCamera/);assert.match(html,/__terrainSelect/);assert.match(html,/id="analysis-boundary"/);assert.match(html,/Immutable analysis boundary/);assert.match(html,/id="analysis-features"/);assert.match(html,/Downloaded terrain features/);assert.match(html,/id="analysis-relationships"/);assert.match(html,/Terrain relationship lines/);assert.match(html,/id="analysis-waypoints"/);assert.match(html,/Downloaded analysis waypoints/);assert.doesNotMatch(renderOfflineMapHtml(result,{analysis:{relationships:false,features:false,waypoints:false}}),/id="analysis-(relationships|features|waypoints)"/);assert.ok(checkpoints.includes("downloading")); });
...
test("offline rendering defaults genuinely optional array fields without hiding malformed arrays", () => {
  ...
  optional.immutable.waypoints = null;
  ...
  const malformedWaypoints = structuredClone(manifest); malformedWaypoints.immutable.waypoints = {};
  assert.throws(() => renderOfflineMapHtml(malformedWaypoints), { name: "OfflineMapModelError", message: "OFFLINE_MAP_WAYPOINTS_INVALID" });
});
